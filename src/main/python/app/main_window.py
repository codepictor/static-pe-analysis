...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def run_model(self):
        ...
        if os.path.isfile(label_path) or os.path.isdir(label_path):
            self.start_run()
            paths_to_pe_files = utils.get_paths_to_pe_files(label_path)
    ...
    def add_row(self, row_idx, path_to_pe_file,
                predicted_label, predicted_score, exception_text):
        """"""
        predicted_score_str = '{:.2f}'.format(predicted_score)
        ...
        log_row = path_to_pe_file + ' : '
        if predicted_label is None:
            log_row += 'NO PREDICTION'
            if exception_text:
                log_row += ', ' + 'NOTE: ' + exception_text
        else:
            if predicted_label:
                log_row += 'WARNING'
            else:
                log_row += 'OK'
            log_row += ', SCORE = ' + predicted_score_str
            log_row += '\n'
